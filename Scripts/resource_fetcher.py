...
class ResourceFetcher:
    ...
    def fetch_and_parse_content(self, resource_url, content_type=None):
        ...
        while attempt < 3:
            ...
            if not response:
                attempt += 1
                print("无法从 {} 获取内容。正在重试...".format(resource_url))
                continue
        ...
        if not response:
            print("无法从 {} 获取内容".format(resource_url))
            return None
        ...
        try:
            ...
            print("解析 {} 内容时出错：{}".format(content_type, e))
    ...
    def download_and_save_file(self, resource_url, destination_path, sha256_hash=None):
        ...
        while attempt < MAX_ATTEMPTS:
            ...
            if os.path.exists(destination_path) and os.path.getsize(destination_path) > 0:
                if sha256_hash:
                    ...
                    print("未提供 SHA256 哈希值。下载文件但不进行验证。")
                    return True
        ...
        print("在 {} 次尝试后仍无法下载 {}。".format(MAX_ATTEMPTS, resource_url))
        return False