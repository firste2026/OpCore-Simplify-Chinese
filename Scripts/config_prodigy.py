...
class ConfigProdigy:
    ...
    def select_audio_codec_layout(self, hardware_report, config=None, controller_required=False):
        ...
        while True:
            ...
            contents.append("- 测试不同的布局以找到最适合您系统的布局。")
            contents.append("")
            content = "\n".join(contents)

            self.utils.adjust_window_size(content)
            self.utils.head("选择编解码器布局 ID", resize=False)
            print(content)
            selected_layout_id = self.utils.request_input("输入您想使用的编解码器布局 ID（默认：{}）：".format(default_layout.id)) or default_layout.id
