...
class ACPIGuru:
    def __init__(self):
        ...
        # 注意：这些是 ACPI _OSI 方法使用的 Windows 版本字符串，不应翻译
        self.pre_patches = (
            {
    ...
    def read_acpi_tables(self, path):
        ...
        if os.path.isdir(path):
            ...
            if len(dsdt_list) > 1:
                print("传递了多个具有 DSDT 签名的文件：")
                for d in self.sorted_nicely(dsdt_list):
                    print(" - {}".format(d))
                print("\n一次只允许一个。请删除上述文件之一后重试。")
                print("")
                #self.u.grab("Press [enter] to return...")
                self.utils.request_input()
                # Restore any prior tables
        ...
        if trouble_dsdt:
            ...
            print("正在检查可用的预补丁...")
            print("正在将 {} 加载到内存...".format(trouble_dsdt))
        ...
        for table_name in self.sorted_nicely(list(self.acpi.acpi_tables)):
            ...
            if not skip_ec:
                ...
                if len(ec_list):
                    ...
                    if log: print(" - 在 {} 中已找到 {}".format(table_name,lpc_name))
                    return lpc_name
...
"""
    ...
    def customize_patch_selection(self):
        while True:
            ...
            contents.append("")
            contents.append("\033[1;93m注意:\033[0m 您可以通过输入逗号分隔的索引来选择多个补丁（例如：'1, 2, 3'）。")
            contents.append("")
