...
class HardwareCustomizer:
        ...
        for device_type, devices in self.hardware_report.items():
            ...
            if device_type == "BIOS":
                ...
                if devices.get("Firmware Type") != "UEFI":
                    print("\n*** BIOS 固件类型不是 UEFI")
                    print("")
                    print("您要为 UEFI 构建 EFI 吗？")
                    print("如果是，请确保更新 BIOS 并在 BIOS 设置中启用 UEFI 启动模式。")
                    print("如果您愿意，仍然可以使用 Legacy。")
                    print("")

                    while True:
                        answer = self.utils.request_input("为 UEFI 构建 EFI？(是/否): ").strip().lower()
                        if answer in ("是", "yes"):
        ...
        if len(devices) > 1:       
            ...
            if device_type == "WiFi" or device_type == "Bluetooth":
                ...
                if _apu_index or _navi_22_indices:
                    print("检测到多个活动 GPU，可能导致 macOS 中的 kext 冲突。")
    ...
    def _select_device(self, device_type, devices, device_groups=None):
        ...
        if device_groups:
            ...
            while True:
                choice = self.utils.request_input(f"选择一个 {device_type} 组合 (1-{len(valid_combinations)}): ")
            ...
            while True:
                choice = self.utils.request_input(f"选择一个 {device_type} 设备 (1-{len(devices)}): ")
